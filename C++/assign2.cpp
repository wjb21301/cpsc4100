...
void noRecusrionQuicksort(int arr[], int n)
{
    ...
    while (s.size() != 0)
    {
        ...
        if (small < large)
        {
            ...
            if (small < pivot - 1)
            {
                s.push({small, pivot - 1});
            }
            if (large > pivot + 1)
            {
                s.push({pivot + 1, large});
            }
        }
    }
}
